...
Scans the source sequence and performs accumulations of state for each
group of keys. The resulting sequence's elements are generated by
projecting each element of the source sequence, its key and the current
state for that key.
