    ...
    public class ScanByTest
    {
        ...
        public void ScanBy()
        {
            ...
            var result =
                source.ScanBy(
                          item => item.First(),
                          key => (Element: default(string), Key: key, State: -1),
                          (state, key, item) => (Element: item, Key: key, State: state.State + 1))
                      .Select(x => (x.Key, (x.Value.Element, x.Value.Key, x.Value.State)));
